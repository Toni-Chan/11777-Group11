...
class CaptionTSVDataset(Dataset):
    def __init__(self, yaml_file, tokenizer=None, add_od_labels=True,
            disable_img_features=False, keep_top_percentage_tag_conf_threshold = 0.3,
    ...
    def get_image_features(self, img_idx):
        ...
        tmp = np.frombuffer(base64.b64decode(feat_info['features']), np.float32
                ).reshape((num_boxes, -1))
    ...
    def get_od_labels(self, img_idx):
        ...
        if self.add_od_labels:
            ...
            if self.keep_top_percentage_tag == 1:
                ...
                label_over_threshold = [l for l in label_info if l['conf'] >= self.keep_top_percentage_tag_conf_threshold]
                sorted_by_conf = sorted(label_over_threshold, key=lambda i: i['conf'], reverse=True)
                # top keep_top_percentage_tag% of object tag that is above the confidence threshold
                label_info_sort_by_conf = sorted_by_conf[:int(len(sorted_by_conf)*self.keep_top_percentage_tag)]
                od_labels = " ".join([l['class'] for l in label_info_sort_by_conf])
        return od_labels
...
def main():
    ...
    parser.add_argument("--disable_img_features", default=False, action='store_true',
                        help="Whether to disable image feature in finetuning state or not")
    parser.add_argument('--keep_top_percentage_tag_conf_threshold', type=float, default=0.3, 
                        help="Confidence threshold k for keep_top_percengate_tag")
    parser.add_argument('--keep_top_percentage_tag', type=float, default=1, 
                        help="Keep input percentage features at inference time given that >= k confidence")
