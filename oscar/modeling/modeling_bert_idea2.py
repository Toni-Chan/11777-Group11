...
class BertForImageCaptioning(CaptionPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.tag_mapping = nn.Linear(768, 768)
        self.txt_mapping = nn.Linear(768, 768)
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask, masked_pos, masked_ids=None, 
            ...
            is_training=True, encoder_history_states=None, cur_len=None):
        outputs = self.bert(input_ids, img_feats=img_feats, attention_mask=attention_mask,  # (batch, len, dim), (batch, dim) -> pooled res
                position_ids=position_ids, token_type_ids=token_type_ids,
                head_mask=head_mask,
                encoder_history_states=encoder_history_states)
        sequence_output = outputs[0][:, :masked_pos.shape[-1], :]
        tag_output = self.tag_mapping(sequence_output[:, 40:, :])
        text_output = self.txt_mapping(sequence_output[:, :40, :])
        ...
        if is_training:
            ...
            class_logits = self.decoder(transformed_output_masked)      # (masked_num, vocab_size)
            tag_logits = torch.matmul(text_output, tag_output.transpose(-1, -2))
            tag_logits = tag_logits / math.sqrt(768)    # (batch, text_len, tag_vocab_size)
            tag_logits = tag_logits[masked_pos[:, :40]==1, :]   # (masked_num, tag_vocab_size)
            cat_logits = torch.cat([class_logits, tag_logits], dim=-1)
            # targets = torch.zeros(cat_logits.shape)
            targets = cat_logits.new(cat_logits.shape).fill_(0)
            count = 0
            for i in range(masked_pos.shape[0]):
                tmp = 0
                for j in range(masked_pos.shape[1]):    # masked_pos: [64, 70]
                    if masked_pos[i][j] == 1:
                        targets[count][masked_ids[i][tmp]] = 1
                    # if masked_ids[i][j] != 0:   # remove padding masks
                    #     targets[i][masked_ids[i][j]] = 1
                        for k in range(40, 70):
                            if token_type_ids[i][k] == 1:
                                if input_ids[i][k] == masked_ids[i][tmp]:
                                    targets[count][class_logits.shape[1] + k - 40] = 1
                        tmp += 1
                        count += 1
            masked_loss = F.binary_cross_entropy_with_logits(cat_logits, targets, reduction="mean")
            masked_loss *= count
            # print("masked_loss", masked_loss)
            # if masked_loss < 0:
            #     print(targets)

            # masked_ids = masked_ids[masked_ids != 0]   # remove padding masks   # (masked_num), groundtruth token id for masked tokens
            # masked_loss = self.loss(class_logits.float(), masked_ids)
            outputs = (masked_loss, class_logits,) + outputs[2:]
        else:
            # inference time focus on tags
            feat = self.transform(sequence_output)      # (batch, total_len, feat_dim)
            pred_token_feat = feat[:, cur_len:cur_len+1, :]     # (batch, 1, feat_dim)
            tag_feat = feat[:, cur_len+1:, :]                   # (batch, tag_size, feat_dim)
            combination = torch.matmul(pred_token_feat, tag_feat.transpose(-1, -2))     # (batch, 1, num_tag)
            class_logits = self.decoder(feat)                   # (batch, total_len, vocab_size)
            # tag_logits = self.decoder(combination)
            class_prob = F.log_softmax(class_logits, dim=-1)        # (batch, total_len, vocab_size)
            tag_prob = F.log_softmax(combination, dim=-1)           # (batch, 1, tag_size)
            # class_logits = self.decoder(self.transform(sequence_output))                # (batch, total_len, vocab_size)
            # outputs = outputs[:2]
            # merge probability distributions
            for b in range(sequence_output.shape[0]):
                for i in range(cur_len+1, sequence_output.shape[1]):
                    tag_id = input_ids[b, i]
                    class_prob[b][cur_len][tag_id] += tag_prob[b][0][i-cur_len-1]
            outputs = (class_prob,) + outputs[2:]
    ...
    def generate(self, img_feats, attention_mask, masked_pos, token_type_ids=None,
        ...
        if position_ids is None:
            ...
            if self.add_od_labels:
                od_labels_posids = torch.arange(        # scott modification
