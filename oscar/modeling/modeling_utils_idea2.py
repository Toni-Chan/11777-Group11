...
class CaptionPreTrainedModel(PreTrainedModel):
    ...
    def _generate_beam_search(
        ...
        while cur_len < max_length:
            model_inputs = self.prepare_inputs_for_generation(input_ids, past=past)
            outputs = self(**model_inputs, cur_len=cur_len)  # (batch_size * num_beams, cur_len, vocab_size)

            # visualization
            if False:
                for sample_idx in range(outputs[1][0].shape[0]):    # batch
                    attention_weights = []
                    for lay_attention in outputs[1]:
                        attention_weights.append(lay_attention[sample_idx].squeeze(0))
                    attention_weights = torch.stack(attention_weights)      # 12 layers, 12(heads)*82*82
                    import numpy as np
                    attention_weights = attention_weights.detach().numpy()
                    np.save('attention_' + str(sample_idx) + '.npy', attention_weights)
                    # np.savetxt('attention_' + str(sample_idx) + '.txt', attention_weights, fmt="%d", delimiter=",")
                print("Wcy")
                outputs = outputs[:1]       # wcy modify
